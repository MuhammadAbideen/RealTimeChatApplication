...
io.on('connection', (socket) => {
    socket.on('joinRoom', ({ username, room }) => {
        ...
        socket.broadcast.to(user.room).emit('message', formatMessage("ChatApp", `${user.username} has joined the room`));
    ...
    socket.on('chatMessage', (msg) => {
        ...
        if (msg.startsWith('/pm')) {
            ...
            if (recipient && recipient.id !== user.id) {
                ...
                socket.emit('message', formatMessage('ChatApp', 'User not found or cannot send private message to yourself.'));
            }
        } else {
    ...
    socket.on('disconnect', () => {
        ...
        if (user) {
            io.to(user.room).emit('message', formatMessage("ChatApp", `${user.username} has left the room`));
